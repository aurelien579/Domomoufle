 ...
 * This animation sequence can be configured to keep repeating or to stop after
 * a specified number of cycles.
 *
 * The size of the Icon is determined to be the largest width or height of any
 * Icon. All other Icons are then aligned within the space available when the
 * Icon is painted.
 *
 * An AnimatedIcon cannot be shared by different components. However, the Icons
 * added to an AnimatedIcon can be shared.
 *
 * The animation sequence is a simple sequential display of each Icon. When the
 * end is reached the animation restarts at the first Icon. Icons are displayed
 * in the order in which they are added. To create custom animation sequences
 * you will need to override the getNextIconIndex() and isCycleCompleted()
 * methods.
 */
public class AnimatedIcon implements Icon, ActionListener, Runnable {
    ...
    /**
     * Calculate the width and height of the Icon based on the maximum width and
     * height of any individual Icon.
     */
     ...
     *
     * @param alignmentY common values TOP, CENTER (default) or BOTTOM although
     * any value between 0.0 and 1.0 can be used
     */
     ...
     * stops. The default is -1, which means the animation is continuous.
     */
     ...
     *
     * @param showFirstIcon true when the first icon is to be displayed, false
     * otherwise
     */
    ...
    /**
     * Pause the animation. The animation can be restarted from the current Icon
...
//
     ...
     * first Icon.
     *
     * Typically this method, along with the isCycleCompleted() method, would be
     * extended to provide a custom animation sequence.
     *
    ...
    /**
     * Check if the currently visible Icon is the last Icon to be displayed in
     * the animation sequence. If so, this indicates the completion of a single
     * cycle. The animation can continue for an unlimited number of cycles or
     * for a specified number of cycles.
     *
     * This implemention checks if the last icon is currently displayed.
     *
     * Typically this method, along with the getNextIconIndex() method, would be
     * extended to provide a custom animation sequence.
     *
     * @param currentIndex the index of the Icon currently displayed
     * @param iconCount the number of Icons to be displayed
